...
void teardown_testing(const char *test_name) {
    list_destroy(&global_function_result_map_head, free_symbol_map_value,
              (void*)0);
    initialize_source_location(&global_last_mock_value_location);
    list_destroy(&global_function_parameter_map_head, free_symbol_map_value,
              (void*)1);
    initialize_source_location(&global_last_parameter_location);
}
...
static void free_symbol_map_value(const void *value,
    ...
    assert_true(value);
    list_destroy(&map_value->symbol_values_list_head,
